-- Add new columns
ALTER TABLE public.auction_records ADD COLUMN IF NOT EXISTS created_by UUID REFERENCES auth.users(id);
ALTER TABLE public.auction_records ADD COLUMN IF NOT EXISTS customer_id UUID REFERENCES public.customers(id);

-- Update RLS for SELECT
DROP POLICY IF EXISTS "Users can view their tenant's auction records" ON public.auction_records;
...
CREATE POLICY "Users can view their tenant's auction records"
    ...
    USING (
        EXISTS (
            ...
            WHERE tu.tenant_id = auction_records.tenant_id
            AND tu.user_id = auth.uid()
            AND (tu.role NOT IN ('staff', 'member') OR auction_records.created_by = auth.uid())
        )
    );

-- Update RLS for UPDATE
DROP POLICY IF EXISTS "Users can update their tenant's auction records" ON public.auction_records;
...
DROP POLICY IF EXISTS "Users can delete their tenant's auction records" ON public.auction_records;
...
-- Function to get auction records with creator email
